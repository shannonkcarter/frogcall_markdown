...
Here, using the tidyverse suite of R packages, I merge the data frames for the eight ponds, gather to long format, sum the six daily observations, and adjust time & date format. 
```{r, echo = T, results = 'hide', warning = F}

## Stack raw data into one df
all_ponds <- rbind(p1, p2, p3, p4, p5, p6, p7, p8)
all_ponds <- subset(all_ponds, subset = (YEAR != 2000)) # 2000 is a partial year, so we drop it

## Gather species to long format, adjust time/date formats, rename columns, and sum 6 daily observations
daily_calls <- all_ponds %>% 
...
The result is a tidy tibble. Each row represents the number of calls in a day for a particular species.
